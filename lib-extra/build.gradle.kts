...
p2deps {
  ...
  into("jdtCompileOnly") {
    p2repo("https://download.eclipse.org/eclipse/updates/4.40/")
...
// Direct cleanup unit tests need the glue classes and their compile-only Eclipse dependencies.
sourceSets.test {
  val jdt = sourceSets.named("jdt").get()
  compileClasspath += jdt.output + jdt.compileClasspath
  runtimeClasspath += jdt.output + jdt.compileClasspath
}
...
cleanUpCoverage {
  classes =
      listOf(
          ...
          "com.diffplug.spotless.extra.glue.jdt.cleanup.*",
      )
  // Bootstrap runs inside the isolated P2 loader and is covered by integration tests.
  excludedClasses = listOf("com.diffplug.spotless.extra.glue.jdt.cleanup.SolsticeBootstrap*")
  tests =
      listOf(
  ...
  classDirectories.from(sourceSets.named("jdt").map { it.output.classesDirs })
  sourceDirectories.from("src/jdt/java")
